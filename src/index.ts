...
app.post('/calendars/webhook', async (c) => {
	...
	const calendarId = await findCalendarIdByNotificationChannelId(c.env.DB, channelId);
	if (!calendarId) {
		throw new HTTPException(500, { message: 'Not found calendar_id from internal' });
	}
	...
	if (isChannelCreated) {
		const { nextSyncToken } = await calendarClient.getCalendarEventsByCalendarId(calendarId);
		if (!nextSyncToken) {
	...
	const nextSyncToken = await findNextSyncTokenByCalendarId(c.env.DB, calendarId);
	if (!nextSyncToken) {
		throw new HTTPException(500, { message: 'Failed to get nextSyncToken from internal' });
	}

	const calendarEvents = await calendarClient.getCalendarEventsByCalendarId(calendarId, nextSyncToken);

	if (calendarEvents.nextSyncToken) {
		await upsertNextSyncTokenByCalendarId(c.env.DB, calendarId, calendarEvents.nextSyncToken);
	}
	const messages = calendarEvents.items.map((item) => {
		const isCreated = sameMinute(new Date(item.created), new Date(item.updated));
...
export default {
	...
	scheduled: async (event: ScheduledEvent, env: Env, _ctx: ExecutionContext) => {
		switch (event.cron) {
			...
			// 毎日0時に実行
			case '0 0 * * *': {
				const calendarClient = await CalendarClient.init(env.GOOGLE_CREDENTIALS);
				...
				for (const result of results) {
					...
					await calendarClient.deleteNotificationChannel(result.id, result.resourceId);

					try {
						const id = crypto.randomUUID();
						const { resourceId, expiration } = await calendarClient.createNotificationChannel(
							id,
							result.calendar_id,
						...
						console.log(`delete channel: ${result.id}, and create channel: ${id}`);
					} catch (error) {}
				}
				break;
			}
			...
			case '1 0 * * *': {
				...
				const { results: notificationChannels } = await findAllNotificationChannels(env.DB);
